...
export default class extends BaseSeeder {
  ...
  public async run() {
    ...
    await this.createRHUser(role, "Teresa Catati", 103, "teresa.catati@sic.gov.ao", "-");
    await this.createRHUser(role, "Simão Eduardo", 104, "simao.eduardo@sic.gov.ao", "-");
    await this.createRHUser(role, "Miguel Xavier", 105, "miguel.xavier@sic.gov.ao", "-");
    await this.createRHUser(role, "Edilson Miguel", 106, "edilson.miguel@sic.gov.ao", "-");
    await this.createRHUser(role, "Madalena Dongoxi", 107, "madalena.dongoxi@sic.gov.ao", "-");
    await this.createRHUser(role, "José Paulino", 108, "jose.paulino@sic.gov.ao", "-");
    await this.createRHUser(role, "Yuri Pegado", 109, "jose.paulino@sic.gov.ao", "-");
    await this.createRHUser(role, "Octavia Calembe", 110, "octavia.calembe@sic.gov.ao", "-");
    await this.createRHUser(role, "Yuri Pegado", 111, "yuri.pegado@sic.gov.ao", "-");
    await this.createRHUser(role, "Franklim Bravo", 112, "franklim.bravo@sic.gov.ao", "-");
  }
}
