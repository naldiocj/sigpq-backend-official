...
export default class UserRepository extends BaseRepository {
   ...
   */
  public async findAll(options: any = { page: null, perPage: null, search: null, searchBy: null, orderBy: null, orderByAscOrDesc: null }) {
    try {
      let query = User
        .query()
        .where('username', '<>', 'Root')
  ...
  public async findBy(input, value) {
    try {
      const user = await Database.query()
        .select("users.*")
        .from("users")
        .innerJoin('sigpq_funcionario_orgaos as sfo', 'sfo.pessoafisica_id', 'users.pessoa_id')
        .where(input, value)
    ...
    try {
      ...
      const userRole = await Database.from({ ur: 'user_roles' })
        .select(
          ...
          'r.nome',
          'r.name',
          Database.raw("DATE_FORMAT(r.created_at, '%d/%m/%Y %H:%i:%s') as created_at"),
          Database.raw("DATE_FORMAT(r.updated_at, '%d/%m/%Y %H:%i:%s') as updated_at")
        )
  ...
  public async storeUserRole(user_id: any, role_id: any, trx: any) {
    console.log('user', user_id, 'role', role_id, ' -> faltou melhorar o registo de role do user');
    const input = {
      user_id,
      role_id
    }
  ...
  public async findUserWithoutAccount(options: any = { page: null, perPage: null, search: null, searchBy: null, orderBy: null, orderByAscOrDesc: null }) {
    try {
      let query = User
        .query()
